...
impl TAtomic {
    ...
    pub fn get_intersection_pairs(&self) -> (Vec<&TAtomic>, Vec<TAtomic>) {
        match self {
            TAtomic::Object(TObject::Named(named)) => {
                if let Some(intersection_types) = named.get_intersection_types() {
                    let mut result = vec![];
                    for intersection_type in intersection_types {
                        result.push(intersection_type);
                    }
            ...
            TAtomic::GenericParameter(parameter) => {
                ...
                for constraint_atomic in &parameter.constraint.types {
                    ...
                    if let TAtomic::GenericParameter(constraint_parameter) = constraint_atomic {
                        let mut new_intersection_types = vec![];
                        let intersection_types = constraint_atomic.get_intersection_pairs();
                        new_intersection_types.extend(intersection_types.1);
                        let mut type_part = self.clone();
                        if let TAtomic::GenericParameter(as_type) = &mut type_part {
                            as_type.constraint.clone_from(&constraint_parameter.constraint);
                        }
            ...
            TAtomic::Reference(reference) => {
                if let Some(intersection_types) = reference.get_intersection_types() {
                    let mut result = vec![];
                    for intersection_type in intersection_types {
                        result.push(intersection_type);
                    }
